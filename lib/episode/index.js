	...
	getTorrents: function () {
		...
		return Promise.all(_.flatten([leetx, piratebay, extratorrent, eztv, torrentapi].map(( service ) => {
			...
			.then(( torrents ) => {
				...
				const properRelease = _.filter(torrents, ( torrent ) => {
					return /proper/i.test(torrent.title);
				});

				// Filter REPACK releases
				const repackRelease = _.filter(torrents, ( torrent ) => {
					return /repack/i.test(torrent.title);
				});

				// Filter all other releases
				const standardRelease = _.difference(torrents, [].concat(properRelease, repackRelease));
